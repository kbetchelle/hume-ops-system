...
export default function ToastBackfillTab() {
  ...
  } = useBackfillJob("toast_orders");

  const { data: totalCount } = useQuery({
    queryKey: ["toast-sales-count"],
    queryFn: async () => {
      const { count, error } = await supabase.from("toast_sales").select("*", { count: "exact", head: true });
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Individual order records from Toast API</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      />
      <BackfillCalendarHeatmap type="toast" refetchTrigger={syncProgress.isRunning} />
    </div>
  );
}

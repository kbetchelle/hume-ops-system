...
export function DevDashboardPanel() {
  ...
  return <div className="flex gap-6 h-full">
      ...
      <div className="w-7/12 flex flex-col">
        <Card ref={noteCardRef} className="border flex flex-col flex-1 cursor-text" onClick={() => setIsEditing(true)}>
          <CardHeader className="pb-3">
            <CardTitle className="text-xs">Latest Edits in Ops System Application</CardTitle>
          </CardHeader>
          <CardContent className="flex-1 flex flex-col min-h-0">
            {isEditing ? <RichTextEditor value={noteContent} onChange={setNoteContent} placeholder="Click to add notes..." minHeight="100%" className="flex-1 py-[6px] border border-primary mx-0 px-[20px]" /> : <div className="prose prose-sm max-w-none text-xs flex-1 overflow-auto [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_a]:text-primary [&_a]:underline border-primary py-[3px] mx-[2px] my-[4px] border" dangerouslySetInnerHTML={{
            ...
            <div className="space-y-0">
              ...
              return sortedPages.map(page => {
                ...
                return <div key={page.id}>
                      ...
                      <PageRow page={page} onStatusChange={handleStatusChange} onRoleChange={handleRoleChange} onDelete={handleDelete} />
                    </div>;
              });
            })()}
            </div>

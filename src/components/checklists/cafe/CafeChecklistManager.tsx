...
import {
  useCafeChecklists,
  useCafeChecklistItems,
  useCreateCafeChecklist,
  useUpdateCafeChecklist,
  useDeleteCafeChecklist,
  useCreateCafeItem,
  useUpdateCafeItem,
  useDeleteCafeItem,
  CafeChecklist,
...
export function CafeChecklistManager() {
  const { toast } = useToast();
  const { data: checklists, isLoading } = useCafeChecklists();
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [editingChecklist, setEditingChecklist] = useState<CafeChecklist | null>(null);
  ...
  const createChecklist = useCreateCafeChecklist();
  const updateChecklist = useUpdateCafeChecklist();
  const deleteChecklist = useDeleteCafeChecklist();
  const createItem = useCreateCafeItem();
  const updateItem = useUpdateCafeItem();
  const deleteItem = useDeleteCafeItem();

  const handleSaveChecklist = async (data: Partial<CafeChecklist>) => {
    try {
      if (editingChecklist) {
  ...
  const handleSaveItem = async (data: Partial<CafeChecklistItem>) => {
    try {
      if (editingItem) {
                          ...
                          <Collapsible key={group} defaultOpen>
                            ...
                            <CollapsibleContent className="pl-1">
                              {groupItems.map((item, idx) => {
                                ...
                                return (
                                <div key={item.id} className={`flex items-center gap-2 p-3 border rounded-lg hover:bg-accent/50 transition-colors ${colorClass}`}>
                                  ...
                                  <div className="flex gap-1">
                                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditingItem(item); setIsItemDialogOpen(true); }}><Edit className="h-3 w-3" /></Button>
                                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleDeleteItem(item.id)}><Trash className="h-3 w-3" /></Button>
                                  </div>
                                </div>
                                );
                              })}

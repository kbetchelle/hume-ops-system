...
BEGIN
  ...
  -- that do NOT have a linked profile (no account at all, or pending)
  FOR sling_rec IN
    ...
    WHERE ss.shift_date BETWEEN CURRENT_DATE AND CURRENT_DATE + 7
      ...
      -- No sling_users row linked to a profile via sling_id
      AND NOT EXISTS (
        SELECT 1 FROM sling_users su
        JOIN profiles p ON p.sling_id = su.id
        WHERE su.sling_user_id = ss.sling_user_id::integer
      )

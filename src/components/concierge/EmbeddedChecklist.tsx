...
import { CheckSquare, ChevronDown, ChevronRight, Clock, Wifi, WifiOff, Cloud, X, Sparkles, Music } from "lucide-react";
...
export function EmbeddedChecklist({ variant = "default" }: EmbeddedChecklistProps = {}) {
  ...
  const { data: shiftSubmission, isLoading: submissionLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.from("checklist_shift_submissions" as any) as any)
        .select("*")
  ...
  const { data: checklistAlerts } = useQuery({
    queryKey: ['checklist-alerts', userData?.id],
    queryFn: async () => {

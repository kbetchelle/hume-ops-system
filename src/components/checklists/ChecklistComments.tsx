...
export function ChecklistComments({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data
      } = await supabase.from("user_roles").select("role").eq("user_id", userData.id).single();
      return data?.role;
    },
    enabled: !!userData?.id
  });
  ...
  const visibleComments = comments?.filter(comment => !comment.is_private || isManagerOrAdmin) || [];
  ...
  return <div className={cn("space-y-3", className)}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {commentCount > 0 && <Badge variant="secondary" className="text-xs">
              {commentCount}
            </Badge>}
        </div>
        {!isAddingComment && <Button variant="ghost" size="sm" onClick={() => setIsAddingComment(true)} className="h-8 text-xs">
            {t("Add Comment", "Agregar Comentario")}
          </Button>}
      </div>

      {/* Add Comment Form */}
      {isAddingComment && <Card className="p-3 space-y-3">
          <Textarea placeholder={t("Enter your comment...", "Ingresa tu comentario...")} value={commentText} onChange={e => setCommentText(e.target.value)} className="min-h-[80px] resize-none" />
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Switch id="private-comment" checked={isPrivate} onCheckedChange={setIsPrivate} disabled={!isManagerOrAdmin} />
              <Label htmlFor="private-comment" className="text-xs flex items-center gap-1 cursor-pointer">
                <Lock className="h-3 w-3" />
                {t("Private", "Privado")}
                {!isManagerOrAdmin && <span className="text-muted-foreground">
      ...
      <div className="space-y-2">
        {isLoading ? <div className="space-y-2">
          ...
          </div> : visibleComments.length === 0 ? <div className="text-center py-6 text-sm text-muted-foreground">
            {t("No comments yet. Be the first to comment!", "Aún no hay comentarios. ¡Sé el primero en comentar!")}
          </div> : visibleComments.map(comment => <Card key={comment.id} className={cn("p-3", comment.is_private && "border-amber-500/50 bg-amber-50/50 dark:bg-amber-950/20")}>
              <div className="flex items-start gap-3">
                <div className="flex-1 space-y-1">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      {comment.is_private && <Badge variant="outline" className="text-xs flex items-center gap-1">
                          <Lock className="h-3 w-3" />
                          {t("Private", "Privado")}
                        </Badge>}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {isManagerOrAdmin && (userData?.id === comment.staff_id || isManagerOrAdmin) && <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => handleDeleteComment(comment.id)}>
                            <X className="h-3 w-3" />
                          </Button>}
                    </div>
                  </div>

...
function ModalPageRow({ page, onStatusChange, onRoleChange, onDelete }: ModalPageRowProps) {
  ...
  return (
    <div className="flex items-center py-3 border-b border-border last:border-b-0 gap-4">
      {/* Page Title */}
      <div className="flex-1 text-sm min-w-0 truncate pl-2">
        {page.page_title}
      </div>
      ...
      <div className="w-28" onDoubleClick={handleRoleDoubleClick}>
        {isEditingRole ? (
          <input
            ...
            className="w-full text-xs text-right bg-transparent border-b border-primary outline-none caret-primary px-1"
          />
        ) : (
          <span className="text-xs text-muted-foreground cursor-text block text-right">
            {page.role_category || "—"}
          </span>
        )}
      </div>

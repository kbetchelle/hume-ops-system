...
function getOrderedConfigs(role: AppRole, context: WalkthroughContext): WalkthroughStepConfig[] {
  ...
  const welcomeText = `Welcome to Hume, ${context.firstName}!\nWe'll show you around ${totalCount} key features to get you started.`;
...
/**
 * Returns WalkthroughStep[] for the overlay. Welcome step uses a dummy selector
 * so the overlay renders it centered with no arrow.
 */
